...
function ratingTemplate(rating) {
  ...
  for (let i = 0; i < 5; i++) {
    if (i < fullStars) {
      ...
      stars += `<span aria-hidden="true" class="icon-empty">☆</span>`;
    }
  }
...
function search() {
  let recipeQuery = document.querySelector("#search").value.toLowerCase();
  ...
  let sortedRecipes = filteredRecipes.sort(function(a, b) {
